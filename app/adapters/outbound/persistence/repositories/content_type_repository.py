...
class ContentTypeRepository:
    ...
    async def update(
        ...
        """
        Atualiza um ContentType existente com carregamento de permissões.

        Args:
            db: Sessão de banco de dados
            content_type_id: ID do tipo de conteúdo a atualizar
            domain_content_type: Objeto do domínio com novos valores

        Returns:
            Objeto ContentType atualizado (modelo de domínio)

        Raises:
            ResourceNotFoundException: Se o tipo de conteúdo não for encontrado
            DatabaseOperationException: Em caso de erro inesperado
        """
        from sqlalchemy.orm import selectinload
        from app.adapters.outbound.persistence.models.user_group.auth_content_type import AuthContentType
        ...
        try:
            ...
            await db.flush()
            await db.refresh(content_type, ['permissions'])
