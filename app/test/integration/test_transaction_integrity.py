...
async def test_transaction_rollback_on_error(db_session: AsyncSession):
    ...
    fetched_user = result.scalars().unique().one()  # Add unique()
...
async def test_transaction_commit_success(db_session: AsyncSession):
    ...
    result = await db_session.execute(stmt)
    users = result.scalars().unique().all()  # Added .unique()
    ...
    updated_user1 = result.scalars().unique().one()  # Added .unique()
    ...
    deleted_user2 = result.scalars().unique().one_or_none()  # Added .unique()
